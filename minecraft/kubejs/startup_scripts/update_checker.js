...
StartupEvents.postInit((event) => {
  if (Platform.isLoaded("bcc") && Platform.isLoaded("kubejstweaks") && Platform.isClientEnvironment()) {
    ...
    KJSTweaks.curseForgeGetEndpoint("v1/mods/" + modPackId + "/files?pageSize=1", Client, (response) => {
      ...
      let cfLatestVersionStr = displayName.toLowerCase($Locale.ROOT).replace("all the mods 11-", "").replace(".zip", "")
      ...
      if (cfLatestVersion.compareTo(currentVersion) > 0) {
        ...
        $SystemToast.add(
          ...
          Text.of("Version ").append(Text.green(cfLatestVersion)).append(" is already available!")
        )
      } else {
